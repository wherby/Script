...
TOKEN ="hcvaN38QGOTcS6tUr2zanOtFT95D"
...
def readFile():
    f=open("C:\\Users\\zhoust\\Documents\\id.csv")
    lines=[]
    failedSource=[]
    designs=[]
    lines=[]
    for line in f:
        ...
        fields=line.split(',')
        source=fields[1]
        design=fields[1][0:-17]+"?version=1"
        fields[1]=fields[1][30:-17]
        move_bubble3(design,source)
    f.close()
...
def move_bubble3(design, source):
    endpoint=config["buble_endpoint"]
...
def get_response2(endpoint, path):
    ...
    #print "headers is "+response.read()
    #print "msg"+str(response.msg)
    #print "body is :" +body
    conn.close()
    return body
...
def getToken():
    global TOKEN
    endpoint=config["columbus_endpoint"]
    endpoint="columbus-local.autodesk.com"
